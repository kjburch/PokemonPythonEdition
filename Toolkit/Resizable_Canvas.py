...
class CanvasImage:
    ...
    def __init__(self, placeholder, path):
        ...
        with warnings.catch_warnings():  # suppress DecompressionBombWarning
            ...
            self.__image = self.path  # open image, but down't load it
        ...
        self.__pyramid = [self.smaller()] if self.__huge else [self.path]
    ...
    def smaller(self):
        ...
        while i < self.imheight:
            ...
            self.__image.close()
            self.__image = self.path  # reopen / reset image
    ...
    def tileClicked(self, x, y, tileSize):
        print("\nPoint Clicked", (x, y), "\nImage Area:", self.canvas.coords(self.container))
        print("Window Area", self.canvas.canvasx(0), self.canvas.canvasy(0))
        cordX1, cordY1, cordX2, cordY2 = self.canvas.coords(self.container)[0], self.canvas.coords(self.container)[1], \
                                         self.canvas.coords(self.container)[2], self.canvas.coords(self.container)[3]
        winX = self.canvas.canvasx(0)
        winY = self.canvas.canvasy(0)
        x = ((x+winX-cordX1) / tileSize) / (cordX2-cordX1) * 400
        y = ((y+winY-cordY1) / tileSize) / (cordY2-cordY1) * 400
        print("Final Tiles: ", (math.floor(x), math.floor(y)), "\n")
...
# filename = 'index.jpg'  # place path to your image here
